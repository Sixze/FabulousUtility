...
public:
	...
	/// This function returns a transform in component space.
	static FTransform GetReferenceBoneTransform(const FReferenceSkeleton& ReferenceSkeleton, FName BoneName,
	                                            const FSkelMeshRefPoseOverride* ReferencePoseOverride = nullptr);

	/// The result of the USkinnedMeshComponent::GetRefPoseOverride() function
	/// can be used as the ReferencePoseOverride argument in this function.
	/// This function returns a transform in component space.
	static FTransform GetReferenceBoneTransform(const FReferenceSkeleton& ReferenceSkeleton, int32 BoneIndex,
	                                            const FSkelMeshRefPoseOverride* ReferencePoseOverride = nullptr);

	/// This function returns a transform in component space.
	static FTransform GetReferenceBoneTransform(const FBoneContainer& BoneContainer, FName BoneName);

	/// This function returns a transform in component space.
	static FTransform GetReferenceBoneTransform(const FBoneContainer& BoneContainer, FCompactPoseBoneIndex BoneIndex);
